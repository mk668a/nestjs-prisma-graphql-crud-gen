...
function transformModelField(dmmfDocument: DmmfDocument) {
  return (field: PrismaDMMF.Field): DMMF.ModelField => {
    ...
    const typeInfo: DMMF.TypeInfo = {
      location,
      isList: field.isList,
      type: dmmfDocument.isModelName(field.type) ? dmmfDocument.getModelTypeName(field.type)! : field.type,
    }
      ...
      fields: inputType.fields
        ...
        .map<DMMF.SchemaArg>((field) => {
          ...
          const isOmitted = !modelField?.isOmitted.input
            ...
            : (modelField.isOmitted.input.includes(InputOmitSetting.Create) && inputType.name.includes('Create')) ||
              (modelField.isOmitted.input.includes(InputOmitSetting.Update) && inputType.name.includes('Update')) ||
              (modelField.isOmitted.input.includes(InputOmitSetting.Where) && inputType.name.includes('Where')) ||
              (modelField.isOmitted.input.includes(InputOmitSetting.OrderBy) && inputType.name.includes('OrderBy'))
          return {
            ...field,
            selectedInputType,
            typeName,
      ...
      fields: outputType.fields
        ...
        .map<DMMF.OutputSchemaField>((field) => {
          ...
          const outputTypeInfo: DMMF.TypeInfo = {
            ...field.outputType,
            type: getMappedOutputTypeName(dmmfDocument, field.outputType.type as string),
          }
...
export function getMappedOutputTypeName(dmmfDocument: DmmfDocument, outputTypeName: string): string {
  ...
  return (mapping: PrismaDMMF.ModelMapping): DMMF.ModelMapping => {
    ...
    const actions = Object.entries(availableActions)
      ...
      .map<DMMF.Action>(([modelAction, fieldName]) => {
        ...
        const argsTypeName = method.args.length > 0 ? `${pascalCase(`${kind}${dmmfDocument.getModelTypeName(mapping.model)}`)}Args` : undefined
...
function selectInputTypeFromTypes(dmmfDocument: DmmfDocument) {
  return (inputTypes: PrismaDMMF.SchemaArgInputType[]): DMMF.SchemaArgInputType => {
    ...
    const selectedInputType =
      ...
      (useUncheckedScalarInputs && possibleInputTypes.find((it) => (it.type as string).includes('Unchecked'))) ||
...
function getMappedActionName(actionName: DMMF.ModelAction, typeName: string, options: GeneratorOptions): string {
...
export function transformEnums(dmmfDocument: DmmfDocument) {
  return (enumDef: PrismaDMMF.DatamodelEnum | PrismaDMMF.SchemaEnum): DMMF.Enum => {
    ...
    const enumValues = enumDef.values as Array<PrismaDMMF.DatamodelEnum['values'][number] | PrismaDMMF.SchemaEnum['values'][number]>
    ...
    return {
      ...
      valuesMap: enumValues.map((enumValue) => {
        ...
        return {
          value: enumValueName,
          name: (modelName && dmmfDocument.getModelFieldAlias(modelName, enumValueName)) || enumValueName,
        }
      }),
    }
  }
...
export function generateRelationModel(dmmfDocument: DmmfDocument) {
  return (model: DMMF.Model): DMMF.RelationModel => {
    ...
    const relationFields = model.fields
      .filter((field) => field.relationName && !field.isOmitted.output && outputType.fields.some((it) => it.name === field.name))
      .map<DMMF.RelationField>((field) => {
        const outputTypeField = outputType.fields.find((it) => it.name === field.name)!
        const argsTypeName = outputTypeField.args.length > 0 ? `${model.typeName}${pascalCase(field.name)}Args` : undefined
